...
describe("[컨트롤러 계층의 통합 테스트] 도서 목록 조회", () => {
  ...
  describe("[100개 도서 - 1000명 회원] 조회 쿼리 비교", () => {
    ...
    describe("부분 조회 (중간 페이지)", () => {
      ...
      it("[선정: 조인 방식] 부분 조회 (중간 페이지)", async () => {
        const res = await request(app)
          .get("/api/books/authorized")
          .query({
            limit: bookLimit,
    ...
    describe("부분 조회 (마지막 페이지)", () => {
      ...
      it("[선정: 조인 방식] 부분 조회 (마지막 페이지)", async () => {
        const res = await request(app)
          .get("/api/books/authorized")
          .query({
            limit: bookLimit,
    ...
    describe("특정 카테고리 식별자 검색", () => {
      ...
      it("[선정: 조인 방식] 특정 카테고리 식별자 검색", async () => {
        const res = await request(app)
          .get("/api/books/authorized")
          .query({
            categoryID,
  ...
  describe("[100개 도서 - 5000명 회원] 조회 쿼리 비교", () => {
    const categoryID = getRandomKey(BookCategoryID);
    const bookSize = 100;
    const userSize = 5000;
    let bookIDs, userIDs, userID, accessToken;
  ...
  describe("[1000개 도서 - 1000명 회원] 조회 쿼리 비교", () => {
    const categoryID = getRandomKey(BookCategoryID);
    const bookSize = 1000;
    const userSize = 1000;
    let bookIDs, userIDs, userID, accessToken;
  ...
  describe("[1000개 도서 - 5000명 회원] 조회 쿼리 비교", () => {
    const categoryID = getRandomKey(BookCategoryID);
    const bookSize = 1000;
    const userSize = 5000;
    let bookIDs, userIDs, userID, accessToken;
